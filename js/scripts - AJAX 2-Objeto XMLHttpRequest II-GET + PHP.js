...
const characters = document.getElementById('selectCharacters')

form.addEventListener('submit', (e) => {
    e.preventDefault()
    getData(characters.value)
    // getData(characters.children[characters.selectedIndex].value) 
        // Esto es los mismo que lo de arriba solo que con mas codigo, pero la funciona y el resultado es el mismo. Aqui lo que estamos haciendo es llamar el valor que corresponde a id
})
...
const getData = (id) => {
    ...
    if(id == undefined) {
        xhr.open('GET', 'php-mysql/marvel.php')
        ...
        xhr.open('GET', `php-mysql/marvel.php?id=${id}`)
